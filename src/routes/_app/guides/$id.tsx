...
export const Route = createFileRoute("/_app/guides/$id")({
  ...
  beforeLoad: async ({
    ...
    if (registerResult.isErr()) {
      await debug(
        `Error registering guide open: ${registerResult.error.message}`
      );
    }
  },
});
  ...
  return (
    <PageContent key={`guide-step-${search.step}`}>
      ...
      >
        ...
        {tabs.map((guide) => (
          <TabsContent
            key={`guide-${guide}-${search.step}`}
            value={guide.toString()}
          >
            <GuidePage
              id={guide}
              stepIndex={getStepClamped(guides.data, params.id, search.step)}
            />
          </TabsContent>
        ))}
      </Tabs>
