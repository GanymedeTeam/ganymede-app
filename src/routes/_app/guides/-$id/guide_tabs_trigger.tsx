...
export function GuideTabsTrigger({ id, currentId }: { id: number; currentId: number }) {
  ...
  const iconElement = guide.node_image ? (
    <DownloadImage src={guide.node_image} className="size-8 shrink-0 rounded object-cover" />
  ) : (
  ...
  return (
    <div className="group/tab relative flex shrink-0">
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <div
              className={cn(
                'relative flex cursor-pointer items-center gap-1.5 whitespace-nowrap px-3 py-1 font-medium text-xs xs:text-sm transition-all sm:px-3 md:px-4',
                isActive
                  ? 'bg-surface-page text-foreground shadow-sm'
                  : 'bg-surface-card text-foreground/75 hover:bg-surface-page/50',
              )}
            >
              <TabsTrigger value={id.toString()} className="!absolute !inset-0 !opacity-0 !w-full !h-full !p-0 !m-0" />
              {iconElement}
      ...
      <button
        className="-translate-y-1/2 invisible absolute top-2 right-0 z-10 cursor-pointer text-primary-foreground group-hover/tab:visible sm:right-1"
        onClick={async (evt) => {
          evt.stopPropagation()
